    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
                <div class="max-w-xl">
                    <section>
                        <form method="POST" action="{{ route('nurse.update') }}" class="space-y-6">
                            @csrf
                            @method('patch')
                            <div>
                                ...
                                <x-text-input id="registration_number" class="block mt-1 w-full" type="text" name="registration_number"
                                              :value="$nurse->registration_number" required autofocus/>
                            ...
                            <div class="mt-4"><x-input-label for="registration_class_id" :value="__('Registration Class')"/>
                                <select name="registration_class" class="block">
                                    <option selected>--Select--</option>
                                    <option value="Registered Practical Nurse">Registered Practical Nurse</option><option value="Registered Nurse">Registered Nurse</option>
                                    <option value="Nurse Practitioner">Nurse Practitioner</option><option value="Graduate">Graduate</option>
                                </select>
                            ...
                            <div class="mt-4">
                                ...
                                <input type="date" id="effective_from" name="effective_from" class="block" value="{{$nurse->effective_from}}">
                            </div>
                            <div class="mt-4">
                                <x-input-label for="expiration_date" :value="__('Expiration Date')"/>
                                <input type="date" id="expiration_date" name="expiration_date" class="block" value="{{$nurse->expiration_date}}">
                            </div>
